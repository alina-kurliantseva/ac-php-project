<?php    
    ...
    function ValidateFriendID($FieldName) {
        $StudentID = trim($FieldName);     
        if (strlen($StudentID) == 0) {
            $errorStudentID = "Student ID field can not be blank.";
        } elseif (User::find_by_id($StudentID) === false) {
            $errorStudentID = "Student ID does not exist.";
        } elseif ($_SESSION['user_id'] == $StudentID) {
            $errorStudentID = "You cannot send a friend request to yourself.";
        } elseif (Friendship::verify_friendship($_SESSION['user_id'], $StudentID, "accepted") === true) {
            $errorStudentID = "You cannot send a friend request to someone who is already your friend.";
        } elseif (Friendship::verify_friendship($StudentID, $_SESSION['user_id'], "request") === true) {  
            Friendship::change_friendship_status($StudentID, $_SESSION['user_id']);
            $errorStudentID = "Congratulations! You are friends now.";
        } else {
            Friendship::make_request($_SESSION['user_id'], $StudentID);
            $errorStudentID = "";
        }
        return $errorStudentID;    
    }    