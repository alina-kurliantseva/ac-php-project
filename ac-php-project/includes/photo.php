	...
	public function save() {
        ...
        if ($numFiles > 2) {
            for ($i = 2; $i < $numFiles; $i++) {
                $ind = strrpos($files[$i], "/");
                $file_name = substr($files[$i], $ind);
                $picture = new Picture ($file_name, $i);
                $pictures["$i"] = $picture;
            }
        }
        return $pictures;
        ...
        public static function get_all_pictures($album_id) {
            global $database;
        ...
        public static function get_all_photos($album_id) {
            ...
            $sql .= "WHERE album_id = '{$album_id}'";
            $the_result_array = self::find_by_query($sql);
            return count((array)$the_result_array);       
        }         
        ...
        function save_uploaded_file ($destinationPath, $j=0) {
            ...
            $tempFilePath = $_FILES['txtUpload']['tmp_name'][$j];
